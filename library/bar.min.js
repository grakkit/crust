const BarColor = Java.type('org.bukkit.boss.BarColor'),
   BarStyle = Java.type('org.bukkit.boss.BarStyle'),
...
export const wrapper = (_, $) => (instance) => {
   const thing = {
      ...
      set color (value) {
         if (('string' == typeof value && (value = $('+').fronts('barColor')[value]), !(value instanceof BarColor)))
            throw 'TypeError: That color is invalid!';
         instance.setColor(value);
      },
      get flags () {
         return _.define($('+').fronts('barFlag'), (entry) => ({
            get: () => instance.hasFlag(entry.value),
            set: (value) => {
               if ('boolean' != typeof value) throw 'TypeError: You must supply a boolean value!';
               value ? instance.addFlag(entry.value) : instance.removeFlag(entry.value);
            }
         }));
      },
      set flags (value) {
         _.keys($('+').fronts('barFlag')).forEach((key) => (thing.flags[key] = value[key] || !1));
      },
      get instance () {
         return instance;
      },
      ...
      set players (value) {
         if (!_.iterable(value)) throw 'TypeError: You must supply an array of player identifiers!';
         {
      ...
      set style (value) {
         if (('string' == typeof value && (value = $('+').fronts('barStyle')[value]), !(value instanceof BarStyle)))
            throw 'TypeError: That style is invalid!';
      ...
      set title (value) {
         if ('string' != typeof value) throw 'TypeError: You must supply a string value!';
...
export const parser = (_, $) => (thing) => {
   ...
   const color = $('+').fronts('barColor')[thing.color],
      style = $('+').fronts('barStyle')[thing.style],
      flags = _.flat(_.entries(thing.flags).map((entry) => entry.value && $('+').fronts('barFlag')[entry.key]));
   return (
      bar
