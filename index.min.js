...
const Block = Java.type('org.bukkit.block.Block'),
   Entity = Java.type('org.bukkit.entity.Entity'),
   Vector = Java.type('org.bukkit.util.Vector'),
   Location = Java.type('org.bukkit.Location'),
   ItemStack = Java.type('org.bukkit.inventory.ItemStack'),
   KeyedBossBar = Java.type('org.bukkit.boss.KeyedBossBar'),
   AttributeModifier = Java.type('org.bukkit.attribute.AttributeModifier'),
   $ = (object, ...args) => {
      if (!_.def(object)) return null;
      switch (typeof object) {
         case 'string':
            const prefix = object[0],
               suffix = object.slice(1);
            switch (prefix) {
               case '~':
         ...
         case 'object':
            ...
            if (void 0 === input) return receiver('empty', object, jx);
            if ('function' == typeof input.instance) return object;
            if (_.iterable(input)) return [ ...object ].map((entry) => $(entry));
            if (_.def(input)) {
               ...
               return receiver(
                  input instanceof KeyedBossBar
                     ? 'bar'
                     : input instanceof AttributeModifier
                       ? 'modifier'
                       : input instanceof Block
                         ...
                         : input instanceof Entity
                           ...
                           : input instanceof ItemStack
                             ? 'item'
                             : input instanceof Location ? 'location' : input instanceof Vector ? 'vector' : 'empty',
                  object,
                  jx
               );
            }
   ...
   bridge = tools.bridge(_, $),
   builder = tools.builder(_, $),
   command = tools.command(_, $),
   event = tools.event(_, $),
   receiver = tools.receiver(_, $),
   jx = {
      bar: builder(bar),
      block: builder(block),
      empty: builder(empty),
...
$('~org.bukkit.Sound'),
   ...
   $('~org.bukkit.entity.EntityType', (value) => 'UNKNOWN' === value.name()),
